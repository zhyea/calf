...
select '鏈煡', '鏈煡'
from dual
where not exists(select 1 from settings where name = '鏈煡');
...
select '鎺ㄨ崘', 'recommend', '鎺ㄨ崘'
from dual
where not exists(select 1 from settings where name = '鎺ㄨ崘');
